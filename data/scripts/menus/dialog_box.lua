...
local function initialize_dialog_box_features(game)
  ...
  local dialog_box = {
    ...
    question_cursor_img = nil,   -- Image of cursor of question choice
  ...
  dialog_box.question_cursor_img = sol.text_surface.create{
    horizontal_aligment = "left",
    vertical_aligment = "top",
    font = dialog_box.font,
      font_size = dialog_box.font_size,
    test = ">",
  }
  ...
  function dialog_box:add_choice(line_index, char_index)
    ...
    if self.selected_answer == nil then
      self:set_selected_answer(line_index)
    end
  end

  function dialog_box:set_selected_answer(line_index)
    self.selected_answer = line_index

    if line_index ~= nil then
      self.question_dst_position.x = self.box_dst_position.x + self.choices[line_index] * 6
      self.question_dst_position.y = self.box_dst_position.y - 8 + line_index * 16
    end
  end
